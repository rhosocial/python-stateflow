...
class OrderOutbox(UUIDMixin, TimestampMixin, ActiveRecord):
    ...
    @classmethod
    def handler_start(cls, event: Any, subprocess: Any) -> "OrderOutbox":
    ...
    @classmethod
    def handler_rollback(cls, event: Any, subprocess: Any) -> "OrderOutbox":
        """Build an outbox item for rolling back a subprocess handler."""
        return cls(
            event_id=event.id,
...
class AsyncOrderOutbox(UUIDMixin, TimestampMixin, AsyncActiveRecord):
    """Async sibling of :class:`OrderOutbox`."""
    ...
    @classmethod
    def handler_start(cls, event: Any, subprocess: Any) -> "AsyncOrderOutbox":
        """Build an outbox item for starting a subprocess handler."""
        return cls(
            event_id=event.id,
            topic=OUTBOX_TOPIC_HANDLER_START,
    ...
    @classmethod
    def handler_rollback(cls, event: Any, subprocess: Any) -> "AsyncOrderOutbox":
        """Build an outbox item for rolling back a subprocess handler."""
        return cls(
            event_id=event.id,
            topic=OUTBOX_TOPIC_HANDLER_ROLLBACK,
